...
describe("secondsUntilNextWake", () => {
  ...
  test("lands on the start of the commute window rather than sleeping past it", () => {
    // 05:10 UTC is 06:10 local. A default 30 minute sleep would wake at 06:40,
    // ten minutes into the window and after the 06:40 look at the display.
    expect(minutesUntilNextWake(utc("2026-08-10T05:10:00Z"))).toBe(20);
  });
  ...
  describe("British Summer Time", () => {
    test("treats 06:35 local as the commute window in summer", () => {
      expect(minutesUntilNextWake(utc("2026-08-10T05:35:00Z"))).toBe(10);
    });

    test("treats the same UTC instant as outside it in winter", () => {
      // 05:35 UTC is 05:35 local in January — an hour before the window opens.
      expect(minutesUntilNextWake(utc("2026-01-12T05:35:00Z"))).toBe(30);
    });

    test("treats 06:35 local as the commute window in winter", () => {
      expect(minutesUntilNextWake(utc("2026-01-12T06:35:00Z"))).toBe(10);
    });
  });
